...
export function GlowButton({ 
    ...
    const handleClick = (e) => {
        ...
        // Auto-shrink on click: start animation and defer onClick until animation ends
    ...
    // When shrink-out is running, call onClick after the animation ends
    useEffect(() => {
        if (!isShrinkingOut || !wrapperRef.current) return;
        const el = wrapperRef.current;
        const onAnimationEnd = (ev) => {
            if (ev.animationName !== 'shrinkOutAnimation') return;
            const pending = pendingClickRef.current;
            pendingClickRef.current = null;
            if (onClick && pending !== null) onClick(pending);
        };
        el.addEventListener('animationend', onAnimationEnd);
        return () => el.removeEventListener('animationend', onAnimationEnd);
    }, [isShrinkingOut, onClick]);
    ...
    return (
        <div ref={wrapperRef} className={glowClasses} style={{ ...glowStyle, ...style }}>
            <button
